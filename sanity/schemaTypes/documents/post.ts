...
import { JOURNAL_CATEGORIES, defaultJournalCategory } from '../../../src/content/journalCategories';
...
export const post = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: JOURNAL_CATEGORIES.map((c) => `${c.title} — ${c.description}`).join('\n'),
      validation: (rule) => rule.required(),
    }),
    defineField({
      name: 'publishedAt',
      ...
      description: 'Détermine la place de l’article dans le Journal : le plus récent en premier.',
      validation: (rule) => rule.required(),
    }),
    defineField({
      name: 'excerpt',
      ...
      rows: 3,
      group: 'meta',
      description: 'Résumé court affiché dans la grille du Journal et dans les partages.',
    ...
    defineField({
      ...
      rows: 3,
      group: 'content',
      description: 'Phrase d’ouverture affichée en grand sous le titre de l’article.',
    ...
    */
    defineField({
      ...
      type: 'richText',
      group: 'content',
      hidden: ({ document }) => ((document?.blocks as unknown[] | undefined)?.length ?? 0) > 0,
      description:
